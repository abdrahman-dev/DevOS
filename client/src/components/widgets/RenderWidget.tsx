...
import type { RenderWidgetData } from '../../types';

interface Props {
  data?: RenderWidgetData;
  collapsed?: boolean;
  onToggleCollapse?: () => void;
}

export default function RenderWidget({ data, collapsed, onToggleCollapse }: Props) {
  if (!data) return null;
  return (
    <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius)', padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 700, flex: 1 }}>Render</span>
        {onToggleCollapse && (
          <button onClick={onToggleCollapse} aria-label={collapsed ? 'Expand Render widget' : 'Collapse Render widget'} style={{ background: 'none', border: 'none', padding: 4, color: 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
        ...
        {!collapsed && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2, ease: 'easeOut' }} style={{ overflow: 'hidden' }}>
            ...
            <div style={{ background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', padding: '12px 16px', textAlign: 'center', marginTop: 8 }}>
              <div style={{ fontSize: 22, fontWeight: 700 }}>{data.services}</div>
              <div style={{ fontSize: 11, color: 'var(--text-2)', marginTop: 1 }}>services</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
