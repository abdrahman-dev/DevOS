...
import type { WakaTimeWidgetData } from '../../types';

interface Props {
  data?: WakaTimeWidgetData;
  collapsed?: boolean;
  onToggleCollapse?: () => void;
}

export default function WakaTimeWidget({ data, collapsed, onToggleCollapse }: Props) {
  if (!data) return null;
  return (
      ...
      <AnimatePresence>
        {!collapsed && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2, ease: 'easeOut' }} style={{ overflow: 'hidden' }}>
            ...
            <div style={{ background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', padding: '10px 12px', textAlign: 'center', marginTop: 8 }}>
              <div style={{ fontSize: 14, fontWeight: 700 }}>{data.totalCodingTime}</div>
              <div style={{ fontSize: 11, color: 'var(--text-2)', marginTop: 1 }}>coding time (7d)</div>
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 8 }}>
              Top language: <strong style={{ color: 'var(--text)' }}>{data.topLanguage}</strong> ({data.topLanguagePercent}%)
            </div>
          </motion.div>
        )}
      </AnimatePresence>
