...
import type { SupabaseWidgetData } from '../../types';

interface Props {
  data?: SupabaseWidgetData;
  collapsed?: boolean;
  onToggleCollapse?: () => void;
}

export default function SupabaseWidget({ data, collapsed, onToggleCollapse }: Props) {
  if (!data) return null;
  return (
    <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius)', padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 700, flex: 1 }}>Supabase</span>
        {onToggleCollapse && (
          <button onClick={onToggleCollapse} aria-label={collapsed ? 'Expand Supabase widget' : 'Collapse Supabase widget'} style={{ background: 'none', border: 'none', padding: 4, color: 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
      ...
      <AnimatePresence>
        {!collapsed && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2, ease: 'easeOut' }} style={{ overflow: 'hidden' }}>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{data.orgName}</div>
